...
export class StudentGradesComponent implements OnInit {
  ...
  async getAllStudentGrades(term) {
    ...
    if (term != 'total') {
      this.grades.push(await this.getStudentGrades(this.user, term));
      this.loading = false;
    } else {
      const prelimGrade = await this.getStudentGrades(this.user, 'prelim');
      const midtermGrade = await this.getStudentGrades(this.user, 'midterm');
      const prefinalsGrade = await this.getStudentGrades(
        this.user,
        'pre-finals'
      );
      const finalsGrade = await this.getStudentGrades(this.user, 'finals');
